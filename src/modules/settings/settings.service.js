...
export class SettingService {
    ...
    async createSetting(createDto) {
        try {
            ...
            const predefined = PREDEFINED_SETTINGS[createDto.key];
            if (predefined) {
    ...
    async getAllSettings(paginationDto) {
    ...
    async updateSettingByKey(key, updateDto) {
        if (!key || typeof key !== 'string') {
            throw new Error("Invalid setting key");
        }
        // Validate against predefined settings if exists
        const predefined = PREDEFINED_SETTINGS[key];
        if (predefined) {
        ...
        const setting = await Setting.findOneAndUpdate({ key }, { value: updateDto.value, updatedAt: new Date() }, { new: true, runValidators: true }).lean();
        if (!setting) {
    ...
    async updateSettingById(id, updateDto) {
        if (!mongoose.Types.ObjectId.isValid(id)) {
            throw new Error("Invalid setting ID format");
        }
        ...
        // Validate against predefined settings if exists
        const predefined = PREDEFINED_SETTINGS[existingSetting.key];
        if (predefined) {
        ...
        const setting = await Setting.findByIdAndUpdate(id, { value: updateDto.value, updatedAt: new Date() }, { new: true, runValidators: true }).lean();
        if (!setting) {
            throw new Error("Setting not found");
        }
        return setting;
    ...
    async bulkUpsertSettings(settings) {
        ...
        for (const setting of settings) {
            try {
                // Validate against predefined settings if exists
                const predefined = PREDEFINED_SETTINGS[setting.key];
                if (predefined) {
                ...
                const updated = await Setting.findOneAndUpdate({ key: setting.key }, { value: setting.value, updatedAt: new Date() }, { new: true, upsert: true, runValidators: true }).lean();
