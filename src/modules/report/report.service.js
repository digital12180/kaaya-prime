...
export class ReportService {
    ...
    async getAllReports(paginationDto) {
    ...
    async searchReportsByTitle(searchTerm, paginationDto) {
    ...
    async updateReport(id, updateDto) {
        ...
        const report = await Report.findByIdAndUpdate(id, { ...updateDto, updatedAt: new Date() }, { new: true, runValidators: true }).lean();
        if (!report) {
            throw new Error("Report not found");
        }
        return report;
    ...
    async updateReportStatus(id, statusDto) {
        if (!mongoose.Types.ObjectId.isValid(id)) {
            throw new Error("Invalid report ID format");
        }
        const report = await Report.findByIdAndUpdate(id, { status: statusDto.status, updatedAt: new Date() }, { new: true, runValidators: true }).lean();
        if (!report) {
            throw new Error("Report not found");
        }
        return report;
    ...
    async getPublishedReports(paginationDto) {
