...
export class UserService {
    ...
    // Create a new user
    async createUser(createDto: ICreateUserDto): Promise<UserResponseDto|any> {
        try {
    ...
    async getAllUsers(paginationDto: IPaginationDto): Promise<{
    ...
    // Get user by ID
    async getUserById(id: string): Promise<UserResponseDto|any> {
    ...
    async searchUsers(searchTerm: string, paginationDto: IPaginationDto): Promise<{
        users: UserResponseDto[]|any;
        total: number;
        page: number;
        limit: number;
    ...
    async updateUserRole(id: string, roleDto: IUpdateRoleDto): Promise<UserResponseDto|any> {
        if (!mongoose.Types.ObjectId.isValid(id)) {
            throw new Error("Invalid user ID format");
        }
        ...
        const user = await User.findByIdAndUpdate(
            id,
            { role: roleDto.role, updatedAt: new Date() },
    ...
    // Delete user
    async deleteUser(id: string): Promise<{ message: string; deletedId: string }> {
        if (!mongoose.Types.ObjectId.isValid(id)) {
            throw new Error("Invalid user ID format");
        }
    ...
    async updateUser(id: string, updateDto: IUpdateUserDto): Promise<UserResponseDto|any> {
        if (!mongoose.Types.ObjectId.isValid(id)) {
            throw new Error("Invalid user ID format");
        }
